...
def _load_cnn_model(path):
    """
    Load a Keras 3.x .keras file safely.
    Tries standalone keras first (correct for models saved with Keras 3),
    then tf.keras as fallback.

    Windows DLL error fix:
      The error "DLL load failed while importing _pywrap_tensorflow_internal"
      means TensorFlow's C++ runtime cannot load on your Windows machine.
      Common causes and fixes:
        1. Missing Visual C++ Redistributable 2019+
           → Download from: https://aka.ms/vs/17/release/vc_redist.x64.exe
        2. Python version mismatch (TF 2.10+ requires Python 3.7–3.10 on Windows)
           → TF 2.13 requires Python ≤ 3.11. Use: py -3.10 -m venv venv
        3. Wrong TensorFlow build (GPU version on a CPU-only machine)
           → pip uninstall tensorflow && pip install tensorflow-cpu
        4. Keras 3 standalone works WITHOUT the TF DLL issue:
           → pip install keras tensorflow-cpu
    """
    # ── Try standalone Keras 3 first (avoids tf DLL entirely if TF not needed) ─
    errors = []
    try:
        import keras
        ...
        errors.append(f"keras.saving: {e1}")
    ...
    try:
        ...
        errors.append(f"keras.models: {e2}")
    ...
    try:
        ...
        errors.append(f"tf.keras: {e3}")

    raise RuntimeError(
        "Could not load CNN model. All backends failed:\n" +
        "\n".join(errors) +
        "\n\nWindows fix: pip install tensorflow-cpu keras>=3.0.0\n"
...
def load_models():
    """Load clinical + CNN models once and cache."""
    import joblib

    CLINICAL_MODEL_PATH = "models/pcos_clinical_model.pkl"
    CNN_MODEL_PATH      = "models/pcos_cnn_model.keras"
    ...
    cnn_model = _load_cnn_model(CNN_MODEL_PATH)
...
def predict_clinical_prob(clinical_data, model_c, imputer, scaler, selector, cols):
    ...
    prob = float(model_c.predict_proba(Xs)[0][1])
    return prob, Xs  # return Xs for SHAP


def predict_cnn_prob(img_pil, cnn_model, img_size=(224, 224)):
    """
    EfficientNetB0 has Rescaling(1/255) built into its graph.
    Feed raw [0,255] float32 — do NOT divide by 255.
    PCOS = index 0  (infected folder sorts alphabetically before notinfected).
    """
    img  = img_pil.convert("RGB").resize(img_size)
    arr  = np.expand_dims(np.array(img, dtype=np.float32), 0)  # [0,255] NO /255
    pred = cnn_model.predict(arr, verbose=0)[0]
    out_units = cnn_model.output_shape[-1]
    if out_units == 1:
        return float(pred[0]), arr   # sigmoid
    return float(pred[0]), arr       # softmax — index 0 = PCOS


def gradcam_heatmap(cnn_model, cnn_arr, img_size=(224, 224)):
    """
    GradCAM for EfficientNetB0 nested inside outer Functional model.

    top_conv lives INSIDE the efficientnetb0 sub-model — not at the outer
    model level. We access it via backbone.get_layer('top_conv'), build a
    backbone-level gradient model, then run the outer head manually.
    """
    try:
        import tensorflow as tf
    except ImportError:
    ...
    # ── Step 1: Get EfficientNetB0 backbone ───────────────────────────
    backbone = None
    try:
    ...
    # ── Step 3: Build backbone-level gradient model ────────────────────
    backbone_grad_model = tf.keras.models.Model(
        inputs=backbone.inputs,
        outputs=[last_conv_layer.output, backbone.output]
    )

    # ── Step 4: Collect outer head layers (skip Input + backbone) ──────
    head_layers = [l for l in cnn_model.layers
                   if type(l).__name__ not in ("InputLayer", "Functional")]

    # ── Step 5: Compute gradients ─────────────────────────────────────
    img_tensor = tf.constant(cnn_arr, dtype=tf.float32)
    with tf.GradientTape() as tape:
        tape.watch(img_tensor)
        conv_out, bb_out = backbone_grad_model(img_tensor, training=False)
        x = bb_out
        for layer in head_layers:
            try:
                x = layer(x, training=False) if isinstance(
                    layer, tf.keras.layers.Dropout) else layer(x)
            except Exception:
                continue
        # PCOS = index 0
        score = x[:, 0]
...
# ─────────────────────────────────────────────────────────────────────
def compute_shap_values(model_c, selector, cols, clinical_Xs):
    """
    Compute SHAP values for the clinical model prediction.
    Returns list of {feature, value, shap_value, direction} sorted by |shap|.
    Returns None if shap is not installed or clinical data unavailable.
    """
    if clinical_Xs is None:
        return None
    try:
        import shap
    except ImportError:
    ...
    try:
        ...
        shap_vals   = explainer.shap_values(clinical_Xs)
    except Exception:
        try:
            def _fn(X): return model_c.predict_proba(X)[:, 1]
            explainer = shap.KernelExplainer(_fn, clinical_Xs)
            shap_vals = explainer.shap_values(clinical_Xs)
        except Exception:
            return None

    # shap_vals: list[class] or ndarray
    sv = shap_vals[1] if isinstance(shap_vals, list) else shap_vals
    sv = sv[0] if sv.ndim == 2 else sv  # (n_features,)

    selected_mask = selector.get_support()
    selected_cols = [c for c, m in zip(cols, selected_mask) if m]

    items = []
    for feat, val, sv_val in zip(selected_cols, clinical_Xs[0], sv):
        items.append({
            "feature":    feat,
            "value":      round(float(val), 3),
            "shap_value": round(float(sv_val), 4),
            "direction":  "↑ Raises PCOS risk" if sv_val > 0 else "↓ Lowers PCOS risk",
        })
    items.sort(key=lambda x: abs(x["shap_value"]), reverse=True)
    return items[:12]


def plot_shap_bar(shap_items):
    """Horizontal bar chart of SHAP values — positive=red, negative=green."""
    if not shap_items:
        return None

    labels = [f"{d['feature']}\n= {d['value']}" for d in shap_items]
    vals   = [d["shap_value"] for d in shap_items]
    colors = ["#D95F72" if v > 0 else "#6BBF8C" for v in vals]
    ...
    ax.axvline(0, color="#B08090", linewidth=0.8, linestyle="--")
    ...
    ax.set_yticklabels(labels, fontsize=7.5, color="#3D2535")
    ax.invert_yaxis()
    ax.spines[["top", "right", "left"]].set_visible(False)
    ax.tick_params(left=False, bottom=False)
    ax.set_xlabel("SHAP value  (impact on PCOS probability)", fontsize=8, color="#7A4D5F")
    ax.set_title("Clinical Feature Importance (SHAP)", fontsize=10,
                 color="#C2637A", fontweight="600", pad=10, fontfamily="serif")

    # Legend
    import matplotlib.patches as mpatches
    ax.legend(handles=[
        mpatches.Patch(color="#D95F72", label="Increases PCOS risk"),
        mpatches.Patch(color="#6BBF8C", label="Decreases PCOS risk"),
        ...
        with st.spinner("🌸 Analysing your data..."):
            ...
            if models_loaded and has_clinical:
                try:
                    ...
                    prob_c, clinical_Xs = predict_clinical_prob(clinical_input, model_c, imputer, scaler, selector, cols)
