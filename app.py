...
# ─────────────────────────────────────────────────────────────────────
# CNN loader — uses standalone keras (keras>=3.0), NOT tf.keras
# Your model was saved with Keras 3.13.2 and CANNOT be loaded with
# tf.keras from TensorFlow 2.x (they are incompatible after Keras 3).
# ─────────────────────────────────────────────────────────────────────
def _load_cnn(path):
    """
    Load a .keras file saved with Keras 3.x.
    Tries standalone `keras` first (correct for your model),
    falls back to `tf.keras` for older models.
    """
    # ── Primary: standalone Keras 3 ──────────────────────────────────
    try:
        import keras
    ...
    # ── Fallback: keras.models.load_model (also Keras 3 API) ─────────
    try:
        import keras
    ...
    # ── Last resort: tf.keras (TF 2.x bundled Keras) ─────────────────
    try:
        ...
        raise RuntimeError(
            f"Could not load CNN model with any backend.\n"
            f"  keras.saving.load_model  → {e1}\n"
            f"  keras.models.load_model  → {e2}\n"
            f"  tf.keras.models.load_model → {e3}\n\n"
            f"FIX: Run  pip install keras>=3.0.0  in your environment."
        )
...
def load_models():
    ...
    # ── Clinical model (sklearn + XGBoost stacking ensemble) ─────────
    ...
    # ── CNN model (EfficientNetB0, Keras 3.13.2, 2-class softmax) ─────
    cnn_model, backend = _load_cnn(CNN_MODEL_PATH)
...
def predict_cnn_prob(img_pil, cnn_model, img_size=(224, 224)):
    """
    Run CNN inference.
    Architecture: EfficientNetB0 + GlobalAvgPool + Dense(2, softmax)
    The model has a built-in Rescaling layer, so input must be
    raw uint8 pixel values [0, 255] — NOT divided by 255.
    Output: index 1 = PCOS probability (softmax, 2 classes)
    """
    img = img_pil.convert("RGB").resize(img_size)
    # ── IMPORTANT: EfficientNetB0 in this model includes a Rescaling
    #    layer internally. Feed raw [0,255] float32, not [0,1]. ──────
    arr  = np.expand_dims(np.array(img, dtype=np.float32), 0)  # [0,255]
    pred = cnn_model.predict(arr, verbose=0)[0]
    out_units = cnn_model.output_shape[-1]
    if out_units == 1:
        # sigmoid output
        return float(pred[0]), arr
    # softmax 2-class: index 0 = No PCOS, index 1 = PCOS
    return float(pred[0]), arr


def gradcam_heatmap(cnn_model, cnn_arr, img_size=(224, 224)):
    """
    GradCAM for EfficientNetB0 backbone.
    Searches inside the nested 'efficientnetb0' sub-model for the last Conv2D.
    cnn_arr must be raw [0,255] float32 (same as predict_cnn_prob output).
    """
    # ── Try to import GradientTape from whichever backend is available ─
    try:
        import tensorflow as tf
        _tape_lib = "tf"
    except ImportError:
    ...
    # ── Find last Conv2D — search top-level AND inside sub-models ─────
    last_conv = None

    def _find_last_conv(model):
        """Walk layers in reverse, including nested sub-models."""
        for layer in reversed(model.layers):
            # Check if this layer has sub-layers (nested Functional model)
            if hasattr(layer, 'layers'):
                result = _find_last_conv(layer)
                if result:
                    return result
            cls = type(layer).__name__
            if 'Conv2D' in cls and 'Depthwise' not in cls:
                return layer
        return None

    last_layer = _find_last_conv(cnn_model)
    if last_layer is None:
        return None

    last_conv = last_layer.name

    # ── Build gradient model ──────────────────────────────────────────
    if _tape_lib == "tf":
        import tensorflow as tf
        # For nested models, get the output from the EfficientNet sub-model
        try:
            backbone = cnn_model.get_layer("efficientnetb0")
            conv_output_layer = backbone.get_layer(last_conv)
            grad_model = tf.keras.models.Model(
                inputs=cnn_model.inputs,
                outputs=[conv_output_layer.output, cnn_model.output]
            )
        except Exception:
            # Fallback: use top-level model
            grad_model = tf.keras.models.Model(
                inputs=cnn_model.inputs,
        ...
        # keras-only path (no tf)
        import keras
        import keras.backend as K
        return None  # GradCAM needs GradientTape — skip if no TF
